...
do {
    ...
    .forEach { sourceURL in
        ...
        try scriptTable.scripts.forEach { script in
            ...
            if targetModificationDate.map({ sourceModificationDate > $0 && modificationDate(from: .currentDirectory().appending(path: "Sources/main.swift")) < $0 && modificationDate(from: .currentDirectory().appending(path: "Sources/main.swift")) < $0 }) ?? true {
